<template>
  <view class="bpm-working-container">
    <!-- 筛选栏 -->
    ...
    <view class="list-section">
      ...
      >
        ...
        <view v-if="approvals.length > 0" class="list-content">
          <view class="approval-item" v-for="item in approvals" :key="item.id">
            <view class="item-header">
              ...
              <text class="item-status status-pending">待办</text>
            </view>
            <view class="item-content">
              ...
              <view class="item-footer">
                <view class="btn-container">
                  <button class="handle-btn" @click="navigateToDetail(item)">办理</button>
                  <button v-if="item.freeToOther === '1'" class="delegate-btn" @click="handleDelegate(item)">委托</button>
                </view>
              </view>
            </view>
          </view>
        ...
        <view v-else-if="!loading && !refreshing" class="empty-state">
          <text class="empty-text">暂无待办数据</text>
        </view>
        <!-- 加载中状态 -->
...
export default {
  ...
  methods: {
    ...
    async getFlowOptions() {
      try {
        const res = await API.bpm.bpmFlow.getDoingBpmFlowList.get({ model: '0' })
        if (res.code === 200 && res.data) {
          // 检查data是否为数组
          if (Array.isArray(res.data)) {
            const flowList = res.data
            if (flowList.length > 0) {
              const options = flowList.map(item => item.label)
              const values = flowList.map(item => item.value)
    ...
    handleBatchCollect() {
      if (this.selectedItems.length === 0) {
        uni.showToast({ title: '请选择要收藏的项目', icon: 'none' })
        return
      }
      // 调用收藏API
      console.log('批量收藏:', this.selectedItems)
    ...
    async loadData() {
      ...
      try {
        ...
        const res = await API.bpm.bpmStepRun.getBpmFlowApprovalList.get(params)
        if (res.code === 200) {
          const responseData = res.data || {}
    ...
    // 处理委托
    handleDelegate(item) {
      this.$emit('handleDelegate', item)
    },
    
    // 处理催办
    handleRemind(item) {
      this.$emit('handleRemind', item)
    }
  }
}
</script>
