    ...
    <view class="list-section">
      ...
      >
        ...
        <view v-if="approvals.length > 0" class="list-content">
          <view class="approval-item" v-for="item in approvals" :key="item.id">
            <view class="item-header">
              ...
              <text class="item-status status-end">已结束</text>
            </view>
        ...
        <view v-else-if="!loading && !refreshing" class="empty-state">
          <text class="empty-text">暂无已结束数据</text>
        </view>
        <!-- 加载中状态 -->
...
export default {
  ...
  methods: {
    ...
    async getFlowOptions() {
      try {
        const res = await API.bpm.bpmFlow.getEndBpmFlowList.get({ model: '0' })
    ...
    async loadData() {
      ...
      try {
        ...
        const res = await API.bpm.bpmFlow.getMyEndBpmFlowList.get({ ...params, model: '0', isDoc: '0' })
        if (res.code === 200) {
          const responseData = res.data || {}
