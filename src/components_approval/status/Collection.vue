    ...
    <view class="list-section">
      ...
      >
        ...
        <view v-if="approvals.length > 0" class="list-content">
          <view class="approval-item" v-for="item in approvals" :key="item.id">
            <view class="item-header">
              ...
              <text class="item-status status-collection">收藏</text>
            </view>
            <view class="item-content">
              ...
              <view class="item-footer">
                <view class="btn-container">
                  ...
                  <button class="uncollect-btn" @click.stop="handleUncollect(item)">取消收藏</button>
                </view>
              </view>
            </view>
          </view>
        ...
        <view v-else-if="!loading && !refreshing" class="empty-state">
          <text class="empty-text">暂无收藏的流程数据</text>
        </view>
        <!-- 加载中状态 -->
...
export default {
  ...
  methods: {
    ...
    async getFlowOptions() {
      try {
        const res = await this.$API.bpm.bpmFlow.getDoingBpmFlowList.get({ model: '0' })
        if (res.code === 200 && res.data && res.data.length > 0) {
          const options = res.data.map(item => item.flowName)
    ...
    async loadData() {
      ...
      try {
        ...
        const res = await API.bpm.bpmCollect.getBpmCollectList.get(params)
        if (res.code === 200) {
    ...
    async handleUncollect(item) {
      try {
        const res = await API.bpm.bpmCollect.removeBpmCollect.post({
          runId: item.runId
        })
