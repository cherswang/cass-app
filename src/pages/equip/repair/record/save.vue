<template>
  ...
  >
    <!-- 弹窗容器 -->
    <view class="repair-accept-popup">
      <!-- 弹窗头部 -->
            ...
            <view class="form-row">
              ...
              >
                <u-input
                    ...
                    :disabled="mode === '验收设备维修单' || form.confirmBy"
                ></u-input>
              </u-form-item>
      ...
      <view class="popup-footer">
        ...
        <u-button
            type="success"
            v-if="form.status == '40' && !form.confirmBy"
            @click="inconfirm"
            class="accept-btn"
        >验收通过</u-button>
...
export default {
  ...
  computed: {
    // 弹窗标题
    popupTitle() {
      return this.mode === "add" ? "添加设备维修单" : this.mode;
    }
  },
  ...
  methods: {
    ...
    async inconfirm() {
      try {
        ...
        if (this.form.repairId != null) {
          // 设置验收人（此处需根据实际用户信息获取）
          // 注意：uni-app中获取用户信息的方式可能不同，请根据实际项目调整
          this.form.confirmBy = uni.getStorageSync('userInfo')?.userName || 'admin';
    ...
    async userSelectedChange(obj) {
      ...
      if (obj != undefined && obj != null) {
        ...
        this.form.status = '20';
        this.form.acceptedBy = obj[0].userName;
        this.form.acceptName = obj[0].nickName;
...
.cancel-btn, .submit-btn, .accept-btn, .reject-btn {
  flex: 1;
  min-width: 120rpx;
}
