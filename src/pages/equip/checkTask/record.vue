  ...
  methods: {
    ...
    async handleCheck(row) {
      try {
        ...
        this.dialog.recordSave = true;
        this.$nextTick(() => {
          this.$refs.recordSave.open(row.planType == "CHECK" ? "点检任务" : "保养任务").setData(res.data);
        });
      } catch (error) {
    ...
    handleRepair(row) {
      ...
      this.$nextTick(() => {
        this.$refs.repairReport.open(row);
      });
    },
    // 关键：弹窗关闭后强制刷新页面布局
    handleDialogClosed() {
      this.dialog.recordSave = false;
