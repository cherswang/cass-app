    ...
    <view class="content" v-if="!loading">
      <view class="section">
        ...
        <view class="info-item">
          ...
          <text class="value">{{ dataDetail.recptDate || '-' }}</text>
        </view>
        <view class="info-item">
          <text class="label">供应商:</text>
          <text class="value">{{ dataDetail.vendorName || '-' }}</text>
        </view>
        <view class="info-item">
          <text class="label">仓库:</text>
          <text class="value">{{ dataDetail.warehouseName+'/'+dataDetail.locationName+'/'+dataDetail.areaName || '-' }}</text>
        </view>
        <view class="info-item">
...
import {getListLine} from '@/api/wms/materialPut'
...
export default {
  ...
  methods: {
    async loadLineDetail() {
      this.loading = true
      try {
        const res = await getListLine({recptId: this.dataDetail.recptId})
        if (res.code === 200) {
