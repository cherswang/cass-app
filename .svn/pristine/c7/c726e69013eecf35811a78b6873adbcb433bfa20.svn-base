        ...
        <scroll-view class="scroll-view" scroll-y @scrolltolower="() => loadMore(0)">
          <view class="list" :style="{ paddingTop: '0' }">
            ...
            <view v-else-if="tableData[0].length === 0" class="empty-container">
              ...
              <text class="empty-text">暂无待出库数据</text>
            </view>
            <!-- 列表内容 -->
            <view v-else class="task-list">
              <view class="task-item" v-for="(task, index) in tableData[0]" :key="task.salseId">
                <view class="task-header">
                  ...
                  <view class="task-no-wrapper">
                    ...
                    <text class="task-no">出库单编号: {{ task.salseCode }}</text>
                  </view>
                ...
                <view class="task-content">
                  ...
                  <view class="content-row">
                    <view class="content-item">
                      ...
                      <text class="value">{{ formatDate(task.salseDate) }}</text>
                    </view>
                  </view>
                  <view class="content-row">
                    <view class="content-item">
                ...
                <view class="task-footer">
                  ...
                  <button class="assign-btn" @click="handleExecute(task)">
        ...
        <scroll-view class="scroll-view" scroll-y @scrolltolower="() => loadMore(1)">
          <view class="list" :style="{ paddingTop: '0' }">
            ...
            <view v-else-if="tableData[1].length === 0" class="empty-container">
              <uni-icons name="none" size="64" color="#ccc"></uni-icons>
              <text class="empty-text">暂无已出库数据</text>
            </view>
            <!-- 列表内容 -->
            <view v-else class="task-list">
              <view class="task-item" v-for="(task, index) in tableData[1]" :key="task.salseId">
                <view class="task-header">
                  <!-- 序号 -->
                  <view class="task-index">{{ index + 1 }}</view>
                  <view class="task-no-wrapper">
...
<script>
import { listProductsalse, getProductsalse, execute } from "@/api/wms/productsalse";
import {updateByBoStatus} from "@/api/pro/deliverApply";
...
export default {
  ...
  onShow() {
    // 监听刷新事件
    uni.$on('refreshProductSalseList', (needRefresh) => {
      if (needRefresh) {
  ...
  onUnload() {
    uni.$off('refreshProductSalseList');
  },
  methods: {
    /** 刷新所有标签页数据 */
    refreshAllTabsData() {
    ...
    fetchTableData(index) {
      ...
      listProductsalse(params)
          ...
          .catch((error) => {
            console.error(`加载${currentTab.name}出库单列表失败:`, error);
    ...
    async handleExecute(row) {
      try {
        ...
        const detailRes = await getProductsalse(row.salseId);
        if (detailRes.code !== 200) {
          throw new Error(detailRes.msg || '获取出库单详情失败');
        }
        const detailData = detailRes.data;

        // 更新发货申请状态（如有）
        if (row.deliverApplyId != null && detailData.materialLines) {
          const totalNetWeight = detailData.materialLines.reduce((sum, item) => {
            const netWeight = Number(item.netWeight) || 0;
            return sum + netWeight;
          }, 0);
          const deliverIds = {
            id: row.deliverApplyId,
            status: '30',
            deliverShippingTime: new Date(),
            actualQuantity: totalNetWeight
          };
          await updateByBoStatus(deliverIds);
        }

        // 执行出库
        const res = await execute(row.salseId);
        if (res.code === 200) {
        ...
        uni.showToast({
          title: error.message || '执行出库失败，请检查库存是否充足',
          icon: 'none'
        });
      } finally {
        uni.hideLoading();
    ...
    async viewDetail(row) {
      try {
        const res = await getProductsalse(row.salseId);
        uni.setStorage({
          key: 'rowData',
          data: res.data,
          success: () => {
            uni.navigateTo({
              url: `/pages/wms/productSalse/detail`
            });
          }
        });
      } catch (error) {
