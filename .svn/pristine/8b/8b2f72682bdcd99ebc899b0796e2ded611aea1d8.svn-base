        ...
        <scroll-view class="scroll-view" scroll-y @scrolltolower="() => loadMore(0)">
          <view class="list" :style="{ paddingTop: '0' }">
            ...
            <view v-else-if="tableData[0].length === 0" class="empty-container">
              ...
              <text class="empty-text">暂无领料数据</text>
            </view>
            <!-- 列表内容 -->
              ...
              <view class="task-item" v-for="(task, index) in tableData[0]" :key="task.id">
                <view class="task-header">
                  ...
                  <view class="task-no-wrapper">
                    ...
                    <text class="task-no">领料单编号: {{ task.issueCode }}</text>
                  </view>
                ...
                <view class="task-content">
                  ...
                  <view class="content-row">
                    <view class="content-item">
                      ...
                      <text class="value">{{ formatDate(task.issueDate) }}</text>
                    </view>
                  </view>
                </view>
                <!-- 任务操作 -->
...
<script>
import { execute, listIssueHeader } from '@/api/wms/productOut'
...
export default {
  ...
  data() {
    return {
      // 标签页配置（待领料/已领料）
      tabsList: [
        { name: '待领料', status: 'PREPARE' },
        { name: '已领料', status: 'FINISHED' }
      ],
      current: 0,
      swiperCurrent: 0,
      searchQuery: '',
  ...
  onShow() {
    uni.$on('refreshProductOutList', (needRefresh) => {
      if (needRefresh) {
  ...
  methods: {
    ...
    fetchTableData(index) {
      ...
      const params = {
        ...
        status: currentTab.status // 按状态筛选待领料/已领料
      }

      listIssueHeader(params)
          .then(res => {
            if (res.code === 200) {
              const data = res.rows || []
          ...
          .catch((error) => {
            console.error(`加载${currentTab.name}领料单列表失败:`, error)
    ...
    viewDetail(row) {
      uni.setStorage({
        ...
        success: () => {
          uni.navigateTo({
            url: `/pages/wms/productOut/detail`
          });
        }
      });
    },
    ...
    async handleAssignOperator(task) {
      ...
      try {
        const res = await execute(task.issueId)
        if (res.code === 200) {
          uni.showToast({ title: '领料成功', icon: 'success', duration: 1500 });
          this.refreshAllTabsData();
        } else {
          uni.showToast({ title: res.msg || '领料失败', icon: 'none' });
        }
      } catch (error) {
        uni.showToast({ title: '网络错误，请重试', icon: 'none' });
        console.error("领料执行失败:", error);
      } finally {
        uni.hideLoading();
      }
    },
